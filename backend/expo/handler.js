...
async function getExpoTime() {
  ...
  const tableResponse = await scanTableWithExpression(table, 'event_name', 'Expo');
...
async function getTeamId(teamName) {
  ...
  const tableResponse = await scanTableWithExpression(table, 'team_name', teamName);

  if (tableResponse[0] === undefined) {
    throw new Error(`Missing team: ${teamName} in ${table}`);
  }
  return tableResponse[0].team_id;
}
 ...
 *                - scheduleFileKey : The s3 key to write the schedule to
 */
      ...
      try {
        ...
        const formattedSubs = devpostSubs.map((item) => {
          ...
          sub.categories = sponsors
            .filter((s) => item.prizes.includes(s.name))
            .map((s) => s.name);
        ...
        // Order the sponsor by how many teams have applied to their prize
        sponsors = sponsors
          .sort((a, b) => b.num_applicants - a.num_applicants)
          ...
          .map((sponsor) => {
            ...
            // Order the teams starting with the teams who have the most demo slots already
            // assigned aka teams that are most full first
            const sortedTeams = [...new Set(
              updatedSponsor.submissions.sort((a, b) => numScheduledDemos(getTeamSubmission(formattedSubs, b))
                - numScheduledDemos(getTeamSubmission(formattedSubs, a))),
            )];

            // Assign the teams who have applied to the sponsor's prize
            sortedTeams.forEach((team) => {
              const teamInfo = getTeamSubmission(formattedSubs, team);
              ...
              if (openBlock >= 0) {
                ...
                console.error(`Team is not assigned ${team} for sponsor ${updatedSponsor.name}\n`
        ...
        const demos = await Promise.all(formattedSubs.map(async (team) => {
          const teamId = await getTeamId(team.title);
