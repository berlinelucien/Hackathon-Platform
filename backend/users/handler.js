...
const sendLoginLinkEmail = async (userId, setRegistrationStatus) => {
  ...
  if (process.env.STAGE !== TESTING_STAGE) {
    ...
    const params = {
      ...
      TemplateData: `{ "firstName":"${firstName}", "loginLink": "${loginLink}" }`,
    };
...
module.exports.send_login_link = withSentry(async (event) => {
  ...
  const result = await sendLoginLinkEmail(emailToUserId(body.email), body.setRegistrationStatus);
  return {
    statusCode: 200,
...
module.exports.add_user = withSentry(async (user) => {
  ...
  await sendLoginLinkEmail(body.id, true);
...
module.exports.upload_resume = async (event) => {
  ...
  return {
    statusCode: 200,
    body: JSON.stringify({ putUrl: s3Result, uploadUrl: `https://bitcamp-2021-resumes.s3.amazonaws.com/${filePath}` }),
    headers: HEADERS,
  };
};
...
module.exports.list_profiles = withSentry(async () => {
  const params = {
    TableName: process.env.USERS_TABLE,
    ProjectionExpression: 'id, full_name',
  };
